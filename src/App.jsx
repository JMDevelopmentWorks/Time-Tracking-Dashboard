    ...
    <div className="app">
      ...
      {dataInfo.map((item, index) => (
        <div className="app__dashboard-card ">
          ...
          <div className={`app__dashboard-card-content ${index === activeCard ? 'activebg' : ''}`}           onMouseDown={() => handleCardMouseDown(index)}
          onMouseUp={() => setActiveCard(null)}>
              <p className="app__dashboard-card-content-title">{item.title} <BiDotsHorizontalRounded className={`icon ${index === activeIcon ? 'active' : ''}`}            onMouseDown={(e) => handleIconMouseDown(index, e)}
            onMouseUp={() => setActiveIcon(null)}/></p>
            {daily && (
